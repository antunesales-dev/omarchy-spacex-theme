...
function parseLaunches(raw) {
  ...
  for (var i = 0; i < results.length; i++) {
    var item = results[i]
    if (!item || !item.net) continue
    var when = new Date(item.net)
    if (isNaN(when.getTime())) continue
    var name = String(item.name || "Launch")
    var parts = name.split(" | ")
    out.push({
      key: dateKey(when.getFullYear(), when.getMonth(), when.getDate()),
      net: item.net,
      local: "",
      name: name,
      short: parts.length > 1 ? parts[1] : name,
      status: (item.status && item.status.abbrev) || "",
      pad: (item.pad && item.pad.name) || "",
      location: "",
...
function launchesOnKey(launches, key) {
  var out = []
  var list = launches || []
  for (var i = 0; i < list.length; i++) {
    if (list[i] && list[i].key === key) out.push(list[i])
  }
  return out
}

function launchesInMonth(launches, year, month) {
  var out = []
  var list = launches || []
  for (var i = 0; i < list.length; i++) {
    var item = list[i]
    if (item && item.year === year && item.month === month) out.push(item)
  }
  out.sort(function(a, b) { return String(a.net || "").localeCompare(String(b.net || "")) })
  return out
}
