...
Panel {
  ...
  // Upcoming SpaceX flights from Launch Library 2 (The Space Devs).
  property var spacexLaunches: []
  property bool launchesFailed: false
  property string selectedKey: todayKey
  readonly property var monthLaunches: Model.launchesInMonth(spacexLaunches, viewYear, viewMonth)
  readonly property var selectedLaunches: Model.launchesOnKey(spacexLaunches, selectedKey)
  readonly property string launchCachePath: {
    var home = Quickshell.env("HOME") || ""
    return home + "/.cache/omarchy-spacex/launches.json"
  }
  ...
  function applyLaunchPayload(raw) {
    var parsed = Model.parseLaunches(raw)
    if (parsed.length > 0) {
      root.spacexLaunches = parsed
      root.launchesFailed = false
    } else if (root.spacexLaunches.length === 0) {
      root.launchesFailed = true
    }
  }

  function launchDot(key) {
    return Model.launchesOnKey(root.spacexLaunches, key).length > 0
  }
  ...
  function selectedDayLabel() {
    var parts = String(root.selectedKey || "").split("-")
    if (parts.length !== 3) return ""
    var d = new Date(Number(parts[0]), Number(parts[1]) - 1, Number(parts[2]))
    return Qt.formatDate(d, "dddd d MMMM")
  }

  function formatLaunchTime(item) {
    if (item && item.local) return item.local
    var d = new Date(item && item.net)
    if (isNaN(d.getTime())) return ""
    return Qt.formatDateTime(d, "ddd d MMM  HH:mm")
  }

  function shortLaunchName(item) {
    if (item && item.short) return item.short
    var s = String((item && item.name) || "")
    var parts = s.split(" | ")
  ...
  function refresh() {
    root.today = new Date()
    root.goToToday()
    root.refreshLaunches()
  }

  function goToToday() {
    root.viewYear = today.getFullYear()
    root.viewMonth = today.getMonth()
    root.selectedKey = Model.keyForDate(today)
  }
  ...
  Process {
    id: launchProc
    command: ["bash", Quickshell.env("HOME") + "/.config/omarchy/themes/spacex/scripts/fetch-spacex-launches.sh"]
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: root.applyLaunchPayload(text)
    }
    onExited: function(code) {
      if (code !== 0 && root.spacexLaunches.length === 0)
        root.launchesFailed = true
    }
  }

  FileView {
    path: root.launchCachePath
    printErrors: false
    onLoaded: root.applyLaunchPayload(text())
  }

  Component.onCompleted: root.refreshLaunches()
      ...
      Flickable {
        ...
        Column {
                ...
                Row {
                  ...
                  Repeater {
                    ...
                    Rectangle {
                      ...
                      Rectangle {
                        visible: root.launchDot(modelData.key)
                        width: 4
                        height: 4
                        radius: 0
                        anchors.horizontalCenter: parent.horizontalCenter
          ...
          Column {
            ...
            Text {
              anchors.horizontalCenter: parent.horizontalCenter
              text: root.selectedDayLabel().toUpperCase()
              color: root.contentForeground
              font.family: root.contentFontFamily
              font.pixelSize: Style.font.body
              font.letterSpacing: 1
            ...
            Text {
              visible: root.selectedLaunches.length === 0
              anchors.horizontalCenter: parent.horizontalCenter
            ...
            Text {
              visible: root.monthLaunches.length > 0
              anchors.horizontalCenter: parent.horizontalCenter
            ...
            Repeater {
              ...
              Row {
                ...
                Text {
                  ...
                  text: root.formatLaunchTime(modelData)
                  color: modelData.key === root.selectedKey ? "#FFFFFF" : root.contentForeground
                  font.family: root.contentFontFamily
                  font.pixelSize: Style.font.caption
                }
