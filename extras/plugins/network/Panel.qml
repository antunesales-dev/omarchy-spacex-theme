...
Panel {
  ...
  readonly property var defaultConnectionPhrases: [
    "Wiring bits",
    "Handling packets",
    "Sorting frames",
    "Hauling bytes",
  ...
  readonly property var starlinkPhrases: [
    "Bending light",
    "Talking to the constellation",
    "Phased array locked",
    "Dish on station",
    "Downlinking from the mesh",
    "Steering the beam",
    "Holding the uplink",
  ]
  readonly property string starlinkSsid: {
    var s = ""
    if (info && info.ssid) s = String(info.ssid)
    else if (connectedWifiNetwork && connectedWifiNetwork.name) s = String(connectedWifiNetwork.name)
    return s
  }
  readonly property bool isStarlink: {
  ...
  KeyboardPanel {
    ...
    Column {
      ...
      // Starlink / Starklink easter egg — only when that SSID is live.
      Item {
        visible: root.isStarlink
        width: parent.width
        implicitHeight: eggCol.implicitHeight + Style.space(14)
        ...
        Canvas {
          ...
          onPaint: {
            ...
            for (var i = 0; i < 28; i++) {
              ...
              var x = (seed % Math.max(1, width))
              seed = (seed * 1103515245 + 12345) & 0x7fffffff
              var y = (seed % Math.max(1, height))
